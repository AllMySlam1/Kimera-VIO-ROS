...
RosOnlineDataProvider::RosOnlineDataProvider(const VioParams& vio_params)
    : RosDataProviderInterface(vio_params), it_(nullptr),
      frame_count_(FrameId(0)), left_img_subscriber_(), right_img_subscriber_(),
      left_cam_info_subscriber_(), right_cam_info_subscriber_(), sync_img_(),
      sync_cam_info_(), imu_subscriber_(), gt_odom_subscriber_(),
      reinit_flag_subscriber_(), reinit_pose_subscriber_(), imu_queue_(),
      imu_async_spinner_(nullptr), async_spinner_(nullptr)
{
    ...
    if (vio_params_.backend_params_->autoInitialize_ == 0) {
        ...
        gt_odom_subscriber_ =
            nh_.subscribe("gt_odom", kMaxGtOdomQueueSize,
                          &RosOnlineDataProvider::callbackGtOdomOnce, this);
        ...
        if (!gt_init_pose_received_) {
            LOG(ERROR) << "Missing ground-truth pose while trying for "
                       << (current - start).toSec() << " seconds.\n"
                       << "Enabling autoInitialize and continuing without "
                          "ground-truth "
                          "pose.";
    ...
    if (use_online_cam_params) {
        LOG(WARNING) << "Using online camera parameters instead of YAML "
                        "parameter files.";

        static constexpr size_t kMaxCamInfoQueueSize             = 10u;
        static constexpr size_t kMaxCamInfoSynchronizerQueueSize = 10u;
        left_cam_info_subscriber_.subscribe(nh_, "left_cam/camera_info",
                                            kMaxCamInfoQueueSize);
        right_cam_info_subscriber_.subscribe(nh_, "right_cam/camera_info",
                                             kMaxCamInfoQueueSize);

        sync_cam_info_ =
            VIO::make_unique<message_filters::Synchronizer<sync_pol_info>>(
                sync_pol_info(kMaxCamInfoSynchronizerQueueSize),
                left_cam_info_subscriber_, right_cam_info_subscriber_);

        DCHECK(sync_cam_info_);
        sync_cam_info_->registerCallback(boost::bind(
            &RosOnlineDataProvider::callbackCameraInfo, this, _1, _2));
        ...
        LOG_IF(FATAL, !camera_info_received_)
            ...
            << " - Left cam info topic: "
            << left_cam_info_subscriber_.getTopic() << '\n'
            << " - Right cam info topic: "
            << right_cam_info_subscriber_.getTopic();
    }
    ...
    left_img_subscriber_.subscribe(*it_, "left_cam/image_raw",
                                   kMaxImagesQueueSize);
    right_img_subscriber_.subscribe(*it_, "right_cam/image_raw",
                                    kMaxImagesQueueSize);
    ...
    sync_img_ = VIO::make_unique<message_filters::Synchronizer<sync_pol_img>>(
        sync_pol_img(kMaxImageSynchronizerQueueSize), left_img_subscriber_,
    ...
    reinit_flag_subscriber_ =
        nh_.subscribe("reinit_flag", kMaxReinitQueueSize,
                      &RosOnlineDataProvider::callbackReinit, this);
    reinit_pose_subscriber_ =
        nh_.subscribe("reinit_pose", kMaxReinitQueueSize,
                      &RosOnlineDataProvider::callbackReinitPose, this);
    ...
    publishStaticTf(vio_params_.camera_params_.at(0).body_Pose_cam_,
                    base_link_frame_id_, left_cam_frame_id_);
    publishStaticTf(vio_params_.camera_params_.at(1).body_Pose_cam_,
                    base_link_frame_id_, right_cam_frame_id_);
    ...
    if (vio_params_.parallel_run_) {
        ...
        imu_async_spinner_ = VIO::make_unique<ros::AsyncSpinner>(
            kImuSpinnerThreads, &imu_queue_);

        //! Vision Spinner
        // This async spinner will process the regular Global callback queue of
        // ROS.
        static constexpr size_t kGlobalSpinnerThreads = 2u;
        async_spinner_ =
            VIO::make_unique<ros::AsyncSpinner>(kGlobalSpinnerThreads);
    }
    else {
    ...
    const sensor_msgs::ImageConstPtr& right_msg)
{
    ...
    if (!shutdown_) {
        ...
        left_frame_callback_(
            VIO::make_unique<Frame>(frame_count_, timestamp_left, left_cam_info,
        ...
        if (vio_params_.frontend_type_ == VIO::FrontendType::kStereoImu) {
            ...
            right_frame_callback_(VIO::make_unique<Frame>(
                frame_count_, timestamp_right, right_cam_info,
    ...
    const sensor_msgs::CameraInfoConstPtr& right_msg)
{
    ...
    utils::msgCamInfoToCameraParams(left_msg, base_link_frame_id_,
                                    left_cam_frame_id_,
                                    &vio_params_.camera_params_.at(0));
    utils::msgCamInfoToCameraParams(right_msg, base_link_frame_id_,
                                    right_cam_frame_id_,
...
void RosOnlineDataProvider::callbackIMU(const sensor_msgs::ImuConstPtr& imu_msg)
{
