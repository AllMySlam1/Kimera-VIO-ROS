...
RosbagDataProvider::RosbagDataProvider(const VioParams& vio_params)
    : RosDataProviderInterface(vio_params), rosbag_data_(), rosbag_path_(""),
      left_imgs_topic_(""), right_imgs_topic_(""), imu_topic_(""),
      gt_odom_topic_(""), clock_pub_(), imu_pub_(), left_img_pub_(),
      right_img_pub_(), gt_odometry_pub_(),
      ...
      timestamp_last_gt_(std::numeric_limits<Timestamp>::min()), k_(0u),
      k_last_kf_(0u), k_last_imu_(0u), k_last_gt_(0u), log_gt_data_(false),
      is_header_written_poses_vio_(false), output_gt_poses_csv_("traj_gt.csv")
{
...
bool RosbagDataProvider::spin()
{
    ...
    while (k_ < rosbag_data_.left_imgs_.size()) {
        if (nh_.ok() && ros::ok() && !ros::isShuttingDown() && !shutdown_) {
            const Timestamp& timestamp_frame_k =
                rosbag_data_.timestamps_.at(k_);
            ...
            if (timestamp_frame_k > timestamp_last_frame_) {
                ...
                left_frame_callback_(VIO::make_unique<Frame>(
                    k_, timestamp_frame_k, left_cam_info,
                    readRosImage(rosbag_data_.left_imgs_.at(k_))));

                // Send right frame data to Kimera:
                if (vio_params_.frontend_type_ ==
                    VIO::FrontendType::kStereoImu) {
                    CHECK(right_frame_callback_)
                        << "Did you forget to register the right frame "
                           "callback?";
                    right_frame_callback_(VIO::make_unique<Frame>(
                        k_, timestamp_frame_k, right_cam_info,
                        readRosImage(rosbag_data_.right_imgs_.at(k_))));
                }

                VLOG(10) << "Sent left/right images to VIO for frame k = "
                         << k_;

                // Publish VIO output if any.
                // TODO(Toni) this could go faster if running in another thread
                // or node...
                ...
                if (timestamp_frame_k == timestamp_last_frame_) {
                    LOG(WARNING)
                        << "Timestamps for current and previous frames are "
                           "equal! \n"
        ...
        if (!vio_params_.parallel_run_) {
            // Break while loop (but keep increasing k_!) if we run in
            // sequential mode. We actually return instead of break, to avoid
            // re-printing that the rosbag processing has finished.
            return true;
        }
                                     ...
                                     RosbagData*        rosbag_data)
{
    ...
    for (const rosbag::MessageInstance& msg : view) {
        ...
        if (imu_msg != nullptr && msg_topic == imu_topic_) {
            ...
            if (imu_data_timestamp > last_imu_timestamp) {
                ...
                if (imu_data_timestamp - last_imu_timestamp == 0u) {
                    LOG(WARNING)
                        << "IMU timestamps in rosbag are repeated!\n"
                        << "Offending timestamp: " << imu_data_timestamp;
                }
                else {
                    LOG(FATAL)
                        << "IMU timestamps in rosbag are out of order: "
                           "consider "
                        << "re-ordering rosbag: \n"
        ...
        if (img_msg != nullptr) {
            if (start_parsing_stereo) {
                ...
                LOG(WARNING)
                    << "Skipping first frame in rosbag, since IMU data not "
                       "yet available.";
            }
            continue;
        }
    ...
    // Sanity check:
    LOG_IF(FATAL, rosbag_data->left_imgs_.size() == 0 ||
    ...
    LOG_IF(WARNING,
           !gt_odom_topic_.empty() && rosbag_data->gt_odometry_.size() <
...
VioNavState
RosbagDataProvider::getGroundTruthVioNavState(const size_t& k_frame) const
{
