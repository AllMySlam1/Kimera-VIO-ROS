...
KimeraVioRos::KimeraVioRos()
    : nh_private_("~"), vio_params_(nullptr), vio_pipeline_(nullptr),
      ros_display_(nullptr), ros_visualizer_(nullptr), data_provider_(nullptr),
      restart_vio_pipeline_srv_(), restart_vio_pipeline_(false)
{
...
bool KimeraVioRos::runKimeraVio()
{
    ...
    // the data provider.
    // NOTE: had the data provider been destroyed before, the vio would be
    // calling the shutdown function of a deleted object, aka segfault.
    ...
    switch (vio_params_->frontend_type_) {
        case VIO::FrontendType::kMonoImu: {
            vio_pipeline_ = VIO::make_unique<VIO::MonoImuPipeline>(
                *vio_params_, std::move(ros_visualizer_),
...
bool KimeraVioRos::spin()
{
    ...
    if (vio_params_->parallel_run_) {
        // TODO(Toni): Technically, we can spare a thread with online
        // dataprovider since we can simply call .start() on the async spinners
        // at the ctor level
        std::future<bool> data_provider_handle =
            std::async(std::launch::async, &VIO::RosDataProviderInterface::spin,
                       CHECK_NOTNULL(data_provider_.get()));
        std::future<bool> vio_viz_handle =
            std::async(std::launch::async, &VIO::Pipeline::spinViz,
                       CHECK_NOTNULL(vio_pipeline_.get()));
        std::future<bool> vio_pipeline_handle =
            std::async(std::launch::async, &VIO::Pipeline::spin,
                       CHECK_NOTNULL(vio_pipeline_.get()));
        ...
        while (ros::ok() && data_provider_->spin() && vio_pipeline_->spin()) {
            // TODO(Toni): right now this will loop forwever unless ROS dies or
            // Ctrl+C
...
void KimeraVioRos::connectVIO()
{
    ...
    data_provider_->registerImuSingleCallback(std::bind(
        ...
        std::ref(*CHECK_NOTNULL(vio_pipeline_.get())), std::placeholders::_1));
