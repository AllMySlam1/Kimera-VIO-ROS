...
RosVisualizer::spinOnce(const VisualizerInput& viz_input)
{
    ...
    const BackendOutput::ConstPtr& output) const
{
    ...
    size_t i = 0;
    for (const std::pair<LandmarkId, gtsam::Point3>& id_point :
         points_with_id) {
...
void RosVisualizer::publishState(const BackendOutput::ConstPtr& output) const
{
    ...
    for (int i = 0; i < vel_cov_body.rows(); i++) {
        for (int j = 0; j < vel_cov_body.cols(); j++) {
            odometry_msg.twist
                .covariance[i * static_cast<int>(sqrt(
                                    odometry_msg.twist.covariance.size())) +
                            j] = vel_cov_body(i, j);
        }
    }
    // Publish message
    ...
    const BackendOutput::ConstPtr&            backend_output) const
{
    ...
    CHECK(nh_private_.getParam("stereo_ransac_threshold",
