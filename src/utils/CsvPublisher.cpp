...
namespace utils {
          ...
          euroc_data_provider_(nullptr)
    {
        ...
        if (euroc_data_provider_->gt_data_.map_to_gt_.empty()) {
            LOG(ERROR) << "CSV at "
                       << euroc_data_provider_->getDatasetPath().c_str()
                       << " is empty!";
            return;
        }
        euroc_data_provider_->gt_data_.print();
        csv_odometry_map_it_ =
            euroc_data_provider_->gt_data_.map_to_gt_.begin();
        ...
        gt_path_pub_ =
            nh_private_.advertise<nav_msgs::Path>("gt_path", queue_size);
        vio_path_pub_ =
            nh_private_.advertise<nav_msgs::Path>("vio_path", queue_size);
        odometry_sub_ = nh_.subscribe("input_odometry", queue_size,
                                      &CsvPublisher::callbackOdometry, this);
    }

    void
    CsvPublisher::callbackOdometry(const nav_msgs::OdometryConstPtr& odom_msg)
    {
        CHECK(euroc_data_provider_);
        const std::map<Timestamp, VioNavState>& csv_odometry_ =
            euroc_data_provider_->gt_data_.map_to_gt_;
        CHECK(!csv_odometry_.empty())
            << "Parsed CSV odometry should not be empty...";
        ...
        // Discard csv poses up to the current odometry timestamp or until we
        // run out of poses.
        ...
        // TODO(Toni): if we were using an incremental method we might be better
        // off constantly aligning ground-truth with vio?
        ...
        // Solve for R,t to align trajectories using Umeyama's method. (only
        // once).
        if (!T_viow_gtw_) {
            ...
            VLOG(1)
                << "Done calculating GT/VIO alignment transform: gt_T_vio_ = \n"
                << *T_viow_gtw_;
        ...
        gtsam::Vector3 viow_V_gt =
            T_viow_gtw_->rotation().toQuaternion() * gtw_V_gt;

        // Publish Odometry
        nav_msgs::Odometry aligned_odometry;
        fillOdometryMsg(csv_odometry_map_it_->first, viow_P_gt, viow_V_gt,
                        &aligned_odometry);
        aligned_gt_odometry_pub_.publish(aligned_odometry);

        nav_msgs::Odometry gt_odometry;
        fillOdometryMsg(csv_odometry_map_it_->first, gtw_P_gt,
                        csv_odometry_map_it_->second.velocity_, &gt_odometry);
