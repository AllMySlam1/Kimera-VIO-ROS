...
namespace utils {
    ...
    void rosTfToGtsamPose(const geometry_msgs::Transform& tf,
                          gtsam::Pose3*                   pose)
    {
        CHECK_NOTNULL(pose);
        *pose = gtsam::Pose3(
            gtsam::Rot3(gtsam::Quaternion(tf.rotation.w, tf.rotation.x,
                                          tf.rotation.y, tf.rotation.z)),
            gtsam::Point3(tf.translation.x, tf.translation.y,
                          tf.translation.z));
    }
    ...
    void
    msgCamInfoToCameraParams(const sensor_msgs::CameraInfoConstPtr& cam_info,
                             ...
                             CameraParams*      cam_params)
    {
        ...
        cam_params->frame_rate_ =
            0;  // TODO(marcus): is there a way to get this?
        ...
        try {
            cam_tf = t_buffer.lookupTransform(base_link_frame_id, cam_frame_id,
                                              ros::Time(0),
                                              ros::Duration(kTfLookupTimeout));
        }
        catch (tf2::TransformException& ex) {
            LOG(FATAL)
                << "TF for left/right camera frames not available. Either "
                   "publish to "
