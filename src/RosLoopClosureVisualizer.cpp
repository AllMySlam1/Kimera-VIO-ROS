        ...
        if (boost::dynamic_pointer_cast<gtsam::BetweenFactor<gtsam::Pose3>>(
            ...
            const gtsam::BetweenFactor<gtsam::Pose3>& factor =
                *boost::dynamic_pointer_cast<
                    gtsam::BetweenFactor<gtsam::Pose3>>(nfg[i]);
...
pose_graph_tools::PoseGraph RosLoopClosureVisualizer::getPosegraphMsg()
{
    ...
    pose_graph.edges.insert(pose_graph.edges.end(), loop_closure_edges_.begin(),
...
void RosLoopClosureVisualizer::publishTf(const LcdOutput::ConstPtr& lcd_output)
{
    CHECK(lcd_output);
