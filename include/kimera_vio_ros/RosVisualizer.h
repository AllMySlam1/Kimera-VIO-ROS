...
class RosVisualizer : public Visualizer3D {
  ...
  public:
     ...
     * @param viz_input
     */
    VisualizerOutput::UniquePtr
    spinOnce(const VisualizerInput& viz_input) override;
  ...
  protected:
    ...
    virtual void
    publishMesherOutput(const MesherOutput::ConstPtr& output) const;

  private:
    void
    publishTimeHorizonPointCloud(const BackendOutput::ConstPtr& output) const;

    void publishPerFrameMesh3D(const MesherOutput::ConstPtr& output) const;

    // void publishTimeHorizonMesh3D(const MesherOutput::ConstPtr& output)
    // const;
    ...
    void
    publishResiliency(const FrontendOutputPacketBase::ConstPtr& frontend_output,
