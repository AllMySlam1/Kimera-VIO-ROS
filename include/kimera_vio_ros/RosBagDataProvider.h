...
class RosbagDataProvider : public RosDataProviderInterface {
  public:
     ...
     * call the VIO IMU callback (so you need to register the imu cb before),
     * and it will parse the gt odom for initialization if requested.
     */
    void initialize();

    // Returns true if the whole rosbag was successfully played, false if ROS
    // was shutdown before the rosbag finished.
    bool spin() override;

  private:
    // Parse rosbag data
    // Optionally, send a ground-truth odometry topic if available in the
    // rosbag. If gt_odom_topic is empty (""), it will be ignored.
