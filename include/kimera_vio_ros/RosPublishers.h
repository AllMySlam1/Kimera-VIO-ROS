...
class RosPublishers {
  ...
  public:
     ...
     * @param latch_msgs Set to true to send last msg to new subscribers
     * (default: true).
     */
    RosPublishers(const ros::NodeHandle& nh_private,
        ...
        : base_topic_name_(base_topic_name), queue_size_(queue_size),
          latch_msgs_(latch_msgs), nh_private_(nh_private), pubs_()
    {
        // TODO(Toni): add check on PublisherType and NodeHandleType, they can
        // only be either ros::Publisher + ros::NodeHandle or
  ...
  protected:
    // We need the two below because one advertise is templated, the other is
    // not.
