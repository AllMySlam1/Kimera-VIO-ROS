...
class RosOnlineDataProvider : public RosDataProviderInterface {
  ...
  public:
    ...
    inline bool getReinitFlag() const
    {
        return reinit_flag_;
    }
    // Resets the current status of reinitialization flag
    inline void resetReinitFlag()
    {
        reinit_packet_.resetReinitFlag();
    }
  ...
  protected:
     ...
     * It calls the callbacks in the dedicated IMU queue and the rest of
     * callbacks (images) by using ros::spinOnce.
     * @return True all the time.
     */
    bool sequentialSpin();
  ...
  private:
    ...
    std::unique_ptr<message_filters::Synchronizer<sync_pol_info>>
        sync_cam_info_;
